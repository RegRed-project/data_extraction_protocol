...
The [**RegRed**]{colour="#d14024" bg-colour="#fff"} database has six tables that relate to each other through IDs - they will be filled as follows. 
  ...
  - The **data extractors** will extract data manually using the [data_extraction_template]{colour="#fff" bg-colour="#31b0e0"} for the tables `source`, `redlist`, and `location`, and will extract data either manually or automatically, depending on the `extractionType` detected, for the `taxon_assessment` table. They will also have to create a file for mapping `verbatimStatusCode` to a standardised `statusCode`, following the [mapping_sources_template]{colour="#fff" bg-colour="#31b0e0"}.   
  - The **spatial data generator** will be in charge of verifying the `location` table, and generating the spatial polygons for all `redlistLocation`s following the [geographic_entities]{colour="#fff" bg-colour="#3169e0"} standard.   
...
### Golden rules when using the data extraction template
  ...
  - Do not modify the [data_extraction_template]{colour="#fff" bg-colour="#31b0e0"} **under any circumstances**. If you find an error and changes are needed, report it to [Flo]{colour="#fff" bg-colour="#000"}.  
...
### 2.3.1 Dealing with status mapping source (`statusAssessmentTypeCitation`)

All the `verbatimStatusCode`s for the species in our database will be mapped to the latest version of the IUCN `statusCodes`. 

The `redlist` table will have a `statusAssessmentTypeCitation` that points to the status mapping source you are using.   
...
-   This Excel file will store the equivalent values of `statusCode` for `verbatimStatusCode`, according to a specific source. You will create it using the [mapping_sources_template_excel]{colour="#fff" bg-colour="#31b0e0"}.
-   You should name the file as follows: <*status_mapping_source_X.xlsx*>, with *X* = `statusMappingSourceID`.
-   You should store the file in the [status_mapping_sources]{colour="#fff" bg-colour="#3169e0"} folder, and name the folder after the `statusMappingSourceID`.  
...
-   This PDF document will have a verbal description and a list of the sources used and decisions made to define equivalent values of `statusCode` for `verbatimStatusCode` (e.g., to determine that `X1 = CR`). You will create it using the [mapping_sources_template_pdf]{colour="#fff" bg-colour="#31b0e0"}.
-   You should name the file as follows: <*status_mapping_source_X.pdf*>, with *X* = `statusMappingID.` 
-   You should store the file in the [status_mapping_sources]{colour="#fff" bg-colour="#3169e0"} folder with the correct `statusMappingSourceID`.  
...
|   |   |
|---|---|
| `locationID` | fill this value according to the [geographic_entities]{colour="#fff" bg-colour="#31b0e0"} `locationID` value |  
...
| `continent` | fill this value according to the [geographic_entities]{colour="#fff" bg-colour="#31b0e0"} `continent` value |  
| `country` | fill this value according to the [geographic_entities]{colour="#fff" bg-colour="#31b0e0"} `country` value |  
| `countryCode` | fill this value according to the [geographic_entities]{colour="#fff" bg-colour="#31b0e0"} `countryCode` value | 
| `stateProvince` | fill this value according to the [geographic_entities]{colour="#fff" bg-colour="#31b0e0"} `stateProvince` value |  
| `county` | fill this value according to the [geographic_entities]{colour="#fff" bg-colour="#31b0e0"} `county` value |  
| `locality` | fill this value according to the [geographic_entities]{colour="#fff" bg-colour="#31b0e0"} `locality` value | 
