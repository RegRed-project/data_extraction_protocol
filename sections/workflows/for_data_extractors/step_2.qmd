...
### 2.3.1 Dealing with status mapping source

All status mapping sources will be created as separate Excel files with their respective documentation PDF, and stored in our [status_mapping_sources]{colour="#fff" bg-colour="#3169e0"} folder.

If you have a `non-IUCN` status assessment first check if there are any official mapping sources published by the countries. If there aren't, you will have to create the mapping source yourself. You could base your criteria on some previous mapping source (e.g., Belarus statuses are equivalent to Russian ones), but do not copy mapping sources blindly. In all cases, you can check with [Flo]{colour="#fff" bg-colour="#000"} how to define the statuses. Always try to come up with something, so that we can finalise together. 

**Instructions to create the status mapping source**   
Creating the mapping source yourself involves preparing two documents:

1.    The **status_mapping_source_X** table

-   This Excel file will store the equivalent values of `statusCode` for `verbatimStatusCode`, according to a specific source, and you will create it using the [mapping_sources_template]{colour="#fff" bg-colour="#31b0e0"}.
-   You should name the file as follows: <*status_mapping_source_X.xlsx*>, with *X* = `statusMappingID.`
-   The file will be stored in [**RegRed**]{colour="#d14024" bg-colour="#fff"}'s GitHub repository and later archived in Zenodo.  
...
2.    The **status_mapping_source_X** documentation

-   This PDF document will have a list of the sources used to define equivalent values of `statusCode` for `verbatimStatusCode` (e.g., to determine that `X1 = CR`).
-   You should name the file as follows: <*status_mapping_source_X.pdf*>, with *X* = `statusMappingID.` 
-   The PDF document will be stored in [**RegRed**]{colour="#d14024" bg-colour="#fff"}'s GitHub repository and later archived in Zenodo. 
...
-   **Title**: "Mapping of <*place*> status codes from <*year/period*> to the standard of IUCN (2012) version 3.1 (2nd edn)" (e.g. "Mapping of Belarus status codes from 1993 to the standard of IUCN (2012) version 3.1 (2nd edn)", "Mapping of Belarus status codes from 2015 and 2025 to the standard of IUCN (2012) version 3.1 (2nd edn)"). Since some countries or provinces may share the same status mapping, the title should be inclusive.
